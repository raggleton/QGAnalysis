 ...
 */
std::vector<GenJetWithParts> QGAnalysisPreselDataModule::getGenJets(std::vector<GenJetWithParts> * jets, std::vector<GenParticle> * genparticles, float pt_min, float eta_max, float lepton_overlap_dr) {
 ...
 */
std::vector<GenParticle> QGAnalysisPreselDataModule::getGenMuons(std::vector<GenParticle> * genparticles, float pt_min, float eta_max) {
 ...
 *
 */
std::vector<Jet> QGAnalysisPreselDataModule::getMatchedJets(std::vector<Jet> * jets, std::vector<GenJetWithParts> * genjets, float drMax, bool uniqueMatch) {
...
void QGAnalysisPreselDataModule::printGenParticles(const std::vector<GenParticle> & gps, const std::string & label, Color::Code color) {
    if (!PRINTOUT) return;
    for (auto & itr: gps) {
...
std::vector<GenParticle*> QGAnalysisPreselDataModule::print_genjet_genparticles(const GenJetWithParts & jet, std::vector<GenParticle>* genparticles) {
...
void QGAnalysisPreselDataModule::printGenJets(const std::vector<GenJetWithParts> & gps, std::vector<GenParticle>* genparticles, const std::string & label, Color::Code color) {
    if (!PRINTOUT) return;
    for (auto & itr: gps) {
        cout << color << "GenJet";
        if (label != "") {
...
float QGAnalysisPreselDataModule::calcGenHT(const std::vector<GenParticle> & gps) {
    float ht = 0.;
