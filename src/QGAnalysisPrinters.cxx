...
void uhh2examples::print_genjet_genparticles(const GenJetWithParts & jet, const vector<GenParticle>* genparticles) {
...
void uhh2examples::print_jet_pfparticles(const Jet & jet, const vector<PFParticle>* pfparticles) {
  if (jet.daughterIndices().size() == 0) return;
...
void uhh2examples::printMuons(const vector<Muon> & muons, const string & label, Color::Code color) {
  ...
  for (auto & itr: muons) {
    ...
    string selection = "";
    for (int i=0; i<=Muon::standalone; i++) {
      selection += itr.get_bool(static_cast<Muon::bool_id>(i)) ? "1" : "0";
    }
...
void uhh2examples::printElectrons(const vector<Electron> & electrons, const string & label, Color::Code color) {
  ...
  for (auto & itr: electrons) {
    ...
    // create string with selection bools
    string selection = "";
    for (int i=0; i<=Electron::cutBasedElectronHLTPreselection_Summer16_V1; i++) {
