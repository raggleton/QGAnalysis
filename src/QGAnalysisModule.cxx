...
QGAnalysisModule::QGAnalysisModule(Context & ctx){
    ...
    cout << "Hello World from QGAnalysisModule!" << endl;
    ...
    dijet_sel.reset(new DijetSelection()); // see QGAnalysisSelections

    // 3. Set up Hists classes:
    h_nocuts.reset(new QGAnalysisHists(ctx, "NoCuts"));
    h_njet.reset(new QGAnalysisHists(ctx, "Njet"));
    h_dijet.reset(new QGAnalysisHists(ctx, "Dijet"));
...
bool QGAnalysisModule::process(Event & event) {
    ...
    cout << "QGAnalysisModule: Starting to process event (runid, eventid) = (" << event.run << ", " << event.event << "); weight = " << event.weight << endl;
